import { isEqual, memoize, merge, omit, omitBy, pickBy, size, sum } from 'lodash'
...
export default class MetaMetricsController {
  ...
  constructor({
    ...
    chrome.alarms.getAll((alarms) => {
      const hasAlarm = checkAlarmExists(alarms, METAMETRICS_FINALIZE_EVENT_FRAGMENT_ALARM)
  ...
  createEventFragment(options) {
    if (!options.successEvent || !options.category) {
      throw new Error(
        `Must specify success event and category. Success event was: ${options.event}. Category was: ${
          options.category
        }. Payload keys were: ${Object.keys(options)}. ${
          typeof options.properties === 'object' ? `Payload property keys were: ${Object.keys(options.properties)}` : ''
        }`
      )
    }
   ...
   *  view
   */
  trackPage({ name, params, environmentType, page, referrer, actionId }, options) {
    try {
  ...
  async submitEvent(payload, options) {
    ...
    if (payload.sensitiveProperties) {
      ...
      const combinedProperties = merge(payload.sensitiveProperties, payload.properties)
  ...
  validatePayload(payload) {
    ...
    if (!payload.event || !payload.category) {
      throw new Error(
        `Must specify event and category. Event was: ${payload.event}. Category was: ${
          payload.category
        }. Payload keys were: ${Object.keys(payload)}. ${
          typeof payload.properties === 'object' ? `Payload property keys were: ${Object.keys(payload.properties)}` : ''
        }`
      )
    }
  }
  ...
  _buildUserTraitsObject(metamaskState) {
    ...
    const currentTraits = {
      [TRAITS.ADDRESS_BOOK_ENTRIES]: sum(Object.values(metamaskState.addressBook).map(size)),
      [TRAITS.INSTALL_DATE_EXT]: traits[TRAITS.INSTALL_DATE_EXT] || '',
      [TRAITS.LEDGER_CONNECTION_TYPE]: metamaskState.ledgerTransportType,
      [TRAITS.NETWORKS_ADDED]: metamaskState.frequentRpcListDetail.map((rpc) => rpc.chainId),
      [TRAITS.NETWORKS_WITHOUT_TICKER]: metamaskState.frequentRpcListDetail.reduce((networkList, currentNetwork) => {
      ...
      [TRAITS.NUMBER_OF_ACCOUNTS]: Object.values(metamaskState.identities).length,
      [TRAITS.NUMBER_OF_NFT_COLLECTIONS]: this._getAllUniqueNFTAddressesLength(metamaskState.allNfts),
      [TRAITS.NUMBER_OF_NFTS]: this._getAllNFTsFlattened(metamaskState.allNfts).length,
    ...
    if (previousUserTraits && !isEqual(previousUserTraits, currentTraits)) {
      const updates = pickBy(currentTraits, (v, k) => !isEqual(previousUserTraits[k], v))
      this.store.updateState({ previousUserTraits: currentTraits })
      return updates
    }
  ...
  _track(payload, options) {
    const { isOptIn, metaMetricsId: metaMetricsIdOverride, matomoEvent, flushImmediately } = options || {}
    let idType = 'userId'
