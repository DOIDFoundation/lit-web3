...
export abstract class BaseProvider extends SafeEventEmitter {
  ...
  protected _handleAccountsChanged(accounts: unknown[], isEthAccounts = false): void {
    ...
    if (!Array.isArray(accounts)) {
      this._log.error('DOID: Received invalid accounts parameter. Please report this bug.', accounts)
      _accounts = []
    }

    for (const account of accounts) {
      if (typeof account !== 'string') {
        this._log.error('DOID: Received non-string account. Please report this bug.', accounts)
        _accounts = []
        break
      }
    }
    ...
    if (!dequal(this._state.accounts, _accounts)) {
      ...
      if (isEthAccounts && this._state.accounts !== null) {
        this._log.error(`DOID: 'eth_accounts' unexpectedly updated accounts. Please report this bug.`, _accounts)
      }
  ...
  protected _handleUnlockStateChanged({ accounts, isUnlocked }: { accounts?: string[]; isUnlocked?: boolean } = {}) {
    if (typeof isUnlocked !== 'boolean') {
      this._log.error('DOID: Received invalid isUnlocked parameter. Please report this bug.')
      return
    }
