...
export default function createRPCMethodTrackingMiddleware({ trackEvent, getMetricsState, rateLimitSeconds = 60 * 5 }) {
  return function rpcMethodTrackingMiddleware(
    ...
    const rateLimited =
      rateLimitType === RATE_LIMIT_TYPES.RATE_LIMITED && typeof rateLimitTimeouts[method] !== 'undefined'
    ...
    if (shouldTrackEvent) {
      ...
      const event = eventType ? eventType.REQUESTED : EVENT_NAMES.PROVIDER_METHOD_CALLED
      ...
      if (method === MESSAGE_TYPE.PERSONAL_SIGN) {
        ...
        // if (isSIWEMessage) {
        //   properties.ui_customizations = [METAMETRIC_KEY_OPTIONS[METAMETRIC_KEY.UI_CUSTOMIZATIONS].SIWE]
        // }
      }
    ...
    next((callback) => {
      ...
      const event = res.error?.code === 4001 ? eventType.REJECTED : eventType.APPROVED
