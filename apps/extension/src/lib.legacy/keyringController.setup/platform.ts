...
export default class ExtensionPlatform {
  ...
  showTransactionNotification(txMeta: TxMeta, rpcPrefs) {
  ...
  _showConfirmedTransaction(txMeta: TxMeta, rpcPrefs) {
    this._subscribeToNotificationClicked()

    const url = getBlockExplorerLink(txMeta, rpcPrefs)
    const nonce = parseInt(txMeta.txParams.nonce, 16)
    const view = startCase(toLower(getURLHostName(url).replace(/([.]\w+)$/u, '')))

    const title = 'Confirmed transaction'
    const message = `Transaction ${nonce} confirmed! ${url.length ? `View on ${view}` : ''}`
    this._showNotification(title, message, url)
  }

  _showFailedTransaction(txMeta: TxMeta, errorMessage: string) {
    const nonce = parseInt(txMeta.txParams.nonce, 16)
  ...
  async _showNotification(title: string, message: string, url?: string) {
