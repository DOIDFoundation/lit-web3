...
export default class MessageManager extends EventEmitter {
  ...
  securityProviderRequest: Function
  constructor({ metricsEvent, securityProviderRequest } = <Record<string, Function>>{}) {
    super()
   ...
   */
  async addUnapprovedMessageAsync(msgParams: any, req: any) {
    ...
    return await new Promise((resolve, reject) => {
      ...
      this.once(`${msgId}:finished`, (data) => {
        switch (data.status) {
          ...
          case 'rejected':
            return reject(
              ethErrors.provider.userRejectedRequest('DOID Message Signature: User denied message signature.')
            )
          case 'errored':
            return reject(new Error(`DOID Message Signature: ${data.error}`))
          default:
            return reject(new Error(`DOID Message Signature: Unknown problem: ${JSON.stringify(msgParams)}`))
        }
      })
    })
  }
   ...
   */
  async addUnapprovedMessage(msgParams: any, req: any) {
    ...
    const securityProviderResponse = await this.securityProviderRequest(msgData, msgData.type)
