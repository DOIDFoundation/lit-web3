...
import { CaveatTypes } from '~/constants/permissions'
import { LOG_IGNORE_METHODS, LOG_LIMIT, LOG_METHOD_TYPES, WALLET_PREFIX } from './enums'
...
export class PermissionLogController {
  ...
  createMiddleware() {
    return (req, res, next, _end) => {
      ...
      if (!LOG_IGNORE_METHODS.includes(method) && (isInternal || this.restrictedMethods.has(method))) {
  ...
  logRequest(request, isInternal) {
    const activityEntry = {
      ...
      methodType: isInternal ? LOG_METHOD_TYPES.internal : LOG_METHOD_TYPES.restricted,
  ...
  getAccountsFromPermission(perm) {
    ...
    for (const caveat of perm.caveats) {
      if (caveat.type === CaveatTypes.restrictReturnedAccounts && Array.isArray(caveat.value)) {
